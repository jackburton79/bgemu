...
// BG1's paperdolls (BAMs, two pictures each) per animation id and armor level
// 1-4, from GemRB's unhardcoded BG1 pdolls.2da; `size` is the body-size letter
// of the "WP" overlay resrefs, '*' for a doll that takes no colors or overlays.
struct PaperdollEntryBG1 {
	uint16 animation_id;
	const char* names[4];
	char size;
};

static const PaperdollEntryBG1 kPaperdollsBG1[] = {
	...
	{ 0x5203, { "CIMC1INV", "CIMC2INV", "CIMC3INV", "CIMC4INV" }, 'S' },
	...
	{ 0x5213, { "CIMC1INV", "CIMC2INV", "CIMC3INV", "CIMC4INV" }, 'S' },
	...
	{ 0x6203, { "CIMC1INV", "CIMC2INV", "CIMC3INV", "CIMC4INV" }, 'S' },
	{ 0x6204, { "CIMC1INV", "CIMC2INV", "CIMC3INV", "CIMC4INV" }, 'S' },
	...
	{ 0x6213, { "CIMC1INV", "CIMC2INV", "CIMC3INV", "CIMC4INV" }, 'S' },
	{ 0x6214, { "CIMC1INV", "CIMC2INV", "CIMC3INV", "CIMC4INV" }, 'S' },
...
static const PaperdollEntryBG1*
_FindPaperdollBG1(uint16 animationID)
{
	for (const PaperdollEntryBG1& entry : kPaperdollsBG1) {
		if (entry.animation_id == animationID)
			return &entry;
	}
	return nullptr;
}
...
AnimationFactory::PaperdollName(const Actor* actor) const
{
	...
	} else if (const PaperdollEntryBG1* entry = _FindPaperdollBG1(fID)) {
		return entry->names[_PaperdollLevel(actor)];
	}
...
std::string
AnimationFactory::PaperdollSizeCode(const Actor* actor) const
{
	if (Core::Get()->Game() == game::GAME_BALDURSGATE) {
		if (const PaperdollEntryBG1* entry = _FindPaperdollBG1(fID))
			return entry->size == '*' ? std::string() : std::string(1, entry->size);
	}
	return SizeCodeForActor(actor);
}
